    ...
    public static class PackageCleanup
    {
        ...
        private static bool GetDestFolderForMovedAsset(GUID assetGUID, out GUID destFolderGUID)
        {
            ...
            if (index >= 0)
            {
                ...
                string destFolderPath = GUIDToAssetPath(destFolderGUID);
                if (string.IsNullOrEmpty(destFolderPath))
                {
                    Debug.LogError("destFolderPath is null or empty for GUID: " + destFolderGUID);
                    return false;
                }
                string newFolder = Path.GetFullPath(destFolderPath);
